...
is_a = core.Relation(theory=t1, symbol='is-a', arity=2, formula_rep=core.Formula.infix_operator_representation, python_name='is_a', formula_is_proposition=True)
...
axiom_2_2 = core.FreeTextAxiom(theory=t1, symbol='axiom 2.2', axiom_text=f'If 𝐧 is a natural number, then 𝐧++ is a natural number.', capitalizable=True)
...
d_2_1_3 = core.FreeTextDefinition(theory=t1, symbol='definition 2.1.3', text='We define 1 to be the number 0++, 2 to be the number (0++)++, 3 to be the number ((0++)++)++,etc. (In other words, 1 := 0++, 2 := 1++, 3 := 2++, etc. In this text I use "x := y" to denote the statement that xis defined to equal y.)', capitalizable=True)
...
core.FormalDefinition(theory=t1, free_text_definition=d_2_1_3,
    valid_proposition=core.Formula(theory=t1, relation=core.equality, parameters=(three,
        core.Formula(theory=t1, relation=suc, parameters=(two)))))
