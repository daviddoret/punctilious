...
class TestTheory(TestCase):
    def test_get_theory_chain(self):
        ...
        t1 = u.t()
        self.assertEqual(frozenset({t1}), t1.get_theory_chain())
        t2a = u.t(extended_theory=t1)
        t2b = u.t(extended_theory=t1)
        self.assertEqual(frozenset({t1, t2a}), t2a.get_theory_chain())
        self.assertEqual(frozenset({t1, t2b}), t2b.get_theory_chain())
        t3 = u.t(extended_theory=t2a)
        self.assertEqual(frozenset({t1, t2a, t3}), t3.get_theory_chain())

    def test_list_theoretical_objcts_recursively(self):
        p.configuration.echo_default = False
        u = p.UniverseOfDiscourse()
        t1 = u.t()
        self.assertEqual({t1}, set(t1.iterate_theoretical_objcts()))
        ...
        t1.dai(f1, ap1)
        return None
        self.assertEqual({t1, r1, o1}, set(t1.iterate_theoretical_objcts()))
        limit = t1.dai(f2, ap1)
        t1.dai(f3, ap1)
        t2a = u.t(extended_theory=t1, extended_theory_limit=limit)
        t2b = u.t(extended_theory=t1)
        self.assertEqual({t1, t2a}, set(t2a.iterate_theoretical_objcts()))
        self.assertEqual({t1, t2b}, set(t2b.iterate_theoretical_objcts()))
        t2a.dai(f2, ap1)
        a2 = u.axiom(random_data.random_sentence())
        ap2 = t2a.postulate_axiom(a2)
        t2a.dai(f3, ap2)
        t3 = u.t(extended_theory=t2a)
        self.assertEqual({t1, t2a, t3}, set(t3.iterate_theoretical_objcts()))
