...
class TestPL5:
    def test_pl5_with_auto_derivation(self, caplog):
        ...
        assert success
        assert pu.as1.is_valid_proposition_in_theory_1(p=is_a_proposition(x), t=t2)

        t2, success, _ = auto_derive_2(t=t2, conjecture=is_a_proposition(y))
        assert success
        assert pu.as1.is_valid_proposition_in_theory_1(p=is_a_proposition(y), t=t2)

        t2, success, _ = auto_derive_2(t=t2, conjecture=is_a_proposition(x | implies | y))
        assert success
        assert pu.as1.is_valid_proposition_in_theory_1(p=is_a_proposition(x | implies | y), t=t2)

        t3, success, _, _ = auto_derive_4(t=t1, conjecture=is_a_proposition(x | implies | y),
                                          max_recursion=4)
        assert success
        assert pu.as1.is_valid_proposition_in_theory_1(p=is_a_proposition(x | implies | y), t=t3)
