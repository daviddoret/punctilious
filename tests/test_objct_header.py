...
class TestHeader(TestCase):
    ...
    def test_header_2(self):
        ...
        header = p.ObjctHeader(header_reference, category=p.statement_categories.proposition, title=header_title)
        self.assertEqual(f'proposition {header_reference}', str(header))
        self.assertEqual(f'proposition {header_reference}', header.repr())
        self.assertEqual(f'proposition {header_reference} - {header_title}', header.repr_header())
        self.assertEqual(f'proposition {header_reference}', header.repr_reference())
        self.assertEqual(f'Proposition {header_reference}', header.repr(cap=True))
        self.assertEqual(f'Proposition {header_reference} - {header_title}', header.repr_header(cap=True))
        self.assertEqual(f'Proposition {header_reference}', header.repr_reference(cap=True))
