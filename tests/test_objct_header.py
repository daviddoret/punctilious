...
class TestHeader(TestCase):
    def test_header_1(self):
        ...
        header = p.ObjctHeader(header_reference, category=p.statement_categories._inference_rule)
    ...
    def test_header_2(self):
        ...
        header = p.ObjctHeader(header_reference, category=p.statement_categories._inference_rule, title=header_title)
        self.assertEqual(f'axiom {header_reference}', str(header))
        self.assertEqual(f'axiom {header_reference}', header.repr())
