...
class TestValidateFormula(TestCase):
    ...
    def test_validate_unary_function_call_formula(self):
        ...
        o1 = u.o.declare()
        o2 = u.o.declare()
        r1 = u.r.declare(arity=1)
        r2 = u.r.declare(arity=1)
        phi = u.f(r1, o1)
        phi1_formula = pu.validate_formula(u=u, flexible_formula=u.f(r1, o1))
        self.assertTrue(phi.is_formula_syntactically_equivalent_to(phi1_formula))
        phi1_tuple = pu.validate_formula(u=u, flexible_formula=(r1, o1))
        ...
        phi_form_ok_1 = pu.validate_formula(u=u, flexible_formula=phi, form=phi)
        self.assertTrue(phi.is_formula_syntactically_equivalent_to(phi_form_ok_1))
        # Successful form validations
        with u.v('x') as x:
            phi_form_ok_1 = pu.validate_formula(u=u, flexible_formula=phi, form=(r1, x), mask=[x])
            self.assertTrue(phi.is_formula_syntactically_equivalent_to(phi_form_ok_1))
        with u.v('x') as x:
            phi_form_ok_1 = pu.validate_formula(u=u, flexible_formula=phi, form=(x, o1), mask=[x])
            self.assertTrue(phi.is_formula_syntactically_equivalent_to(phi_form_ok_1))
        with u.v('x') as x, u.v('y') as y:
            phi_form_ok_1 = pu.validate_formula(u=u, flexible_formula=phi, form=(x, y), mask=[x, y])
            self.assertTrue(phi.is_formula_syntactically_equivalent_to(phi_form_ok_1))
        # Failed form validations
        with self.assertRaises(pu.PunctiliousException):
            with u.v('x') as x:
                pu.validate_formula(u=u, flexible_formula=phi, form=(r2, x), mask=[x])
        with self.assertRaises(pu.PunctiliousException):
            with u.v('x') as x:
                pu.validate_formula(u=u, flexible_formula=phi, form=(x, o2), mask=[x])
    ...
    def test_validate_binary_infix_formula(self):
        ...
        self.assertEqual('(o1 * o2)',
            pu.validate_formula(u=u, arity=2, flexible_formula=(r1, o1, o2)).rep_formula())
        self.assertEqual('(o1 * o2)',
            pu.validate_formula(u=u, arity=2, flexible_formula=o1 | r1 | o2).rep_formula())
