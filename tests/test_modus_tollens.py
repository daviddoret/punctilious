...
class TestModusTollens(TestCase):
    ...
    def test_modus_tollens_without_variable(self):
        ...
        with self.assertRaises(pu.PunctiliousException) as error:
            t1.i.modus_tollens.infer_formula_statement(p_implies_q=o3 | u.r.implies | o2, not_q=o2)
        self.assertIs(pu.error_codes.error_002_inference_premise_syntax_error,
            error.exception.error_code)
        # Validity error
        with self.assertRaises(pu.PunctiliousException) as error:
            t1.i.modus_tollens.infer_formula_statement(p_implies_q=o3 | u.r.implies | o2,
                not_q=u.r.lnot(o2))
