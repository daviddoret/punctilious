...
class TestFormulaStructureTerms:
    def test_formula_structure_terms_hash(self):
        t1 = afl.FormulaStructureTerms()
        t2 = afl.FormulaStructureTerms()
        assert hash(t1) == hash(t2)
        t3 = afl.FormulaStructureTerms((0, 1, 2, 3,))
        t4 = afl.FormulaStructureTerms((0, 1, 2, 3,))
        assert hash(t3) == hash(t4)
        t5 = afl.FormulaStructureTerms((0, 1, 2, 2,))
        assert not hash(t5) == hash(t3)
        t6 = afl.FormulaStructureTerms((0, 1, 2,))
        assert not hash(t6) == hash(t3)
        t7 = afl.FormulaStructureTerms((0, 1, (1, (2, 3, 4, 5, 6, 7, (7, (1, 2)))), 3,))
        t8 = afl.FormulaStructureTerms((0, 1, (1, (2, 3, 4, 5, 6, 7, (7, (1, 2)))), 3,))
        t9 = afl.FormulaStructureTerms((0, 1, (1, (2, 3, 4, 5, 6, 7, (7, (1, 1)))), 3,))
...
class TestFormulaStructure:
    ...
    def test_formula_structure_01(self):
        s0 = afl.FormulaStructure(0)
        assert s0.is_leaf
        assert s0.is_canonical
        s0b = afl.FormulaStructure(0)
        assert s0 == s0b
        assert id(s0) == id(s0b)
        assert s0 is s0b
        s1 = afl.FormulaStructure(1)
        assert s1.is_leaf
        assert not s1.is_canonical
        assert s1 is not s0
        s2 = afl.FormulaStructure(2)
        assert s2.is_leaf
        assert not s2.is_canonical
        s3 = afl.FormulaStructure(0, (s0,))
        assert not s3.is_leaf
        assert s3.is_canonical
        s4 = afl.FormulaStructure(0, (s1,))
        assert not s4.is_leaf
        assert s4.is_canonical
        s5 = afl.FormulaStructure(0, (s2,))
        assert not s5.is_leaf
        assert not s5.is_canonical
        s6 = afl.FormulaStructure(0, (s2, s5, s1, s0,))
        assert not s6.is_leaf
        assert not s6.is_canonical
        s7 = afl.FormulaStructure(0, (s1, s2, s3, s4, s5, s6,))
        assert not s7.is_leaf
        assert s7.is_canonical
        s7b = afl.FormulaStructure(0, (s1, s2, s3, s4, s5, s6,))
        assert s7 == s7b
        assert id(s7) == id(s7b)
        assert s7 is s7b
        assert s7 is not s5
