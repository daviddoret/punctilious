...
class PythonHashOrder(brl.BinaryRelation):
    r"""This order is super efficient for sorting purposes but
    unranking is non feasible.
    """
    ...
    @classmethod
    def rank(cls, x: FlexibleLabeledRootedPlaneTree) -> int:
        r"""Returns the rank of `x` under this order sequence.
    ...
    def relates(cls, x: FlexibleLabeledRootedPlaneTree, y: FlexibleLabeledRootedPlaneTree) -> bool:
        ...
        x: LabeledRootedPlaneTree = LabeledRootedPlaneTree.from_any(x)
        y: LabeledRootedPlaneTree = LabeledRootedPlaneTree.from_any(y)
        x_hash: int = LabeledRootedPlaneTree.compute_hash(x)
        y_hash: int = LabeledRootedPlaneTree.compute_hash(y)
        if x_hash < y_hash:
            return True
        elif y_hash < x_hash:
            return False
...
class NN0SFirstRPTSecondOrder(brl.BinaryRelation):
...
class LabeledRootedPlaneTree(brl.ClassWithOrder, tuple):
    ...
    @classmethod
    def compute_hash(cls, o: LabeledRootedPlaneTree) -> int:
