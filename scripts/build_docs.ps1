...
$current_step = 1
Write-Host "Script: $( $script ). Step: $( $current_step ) / $( $total_step ). start_venv.ps1"
$environment_directory = & $PSScriptRoot\start_venv.ps1

$current_step = 2
$command = "python -m pip install --upgrade --no-cache-dir pip setuptools"
Write-Host "Script: $( $script ). Step: $( $current_step ) / $( $total_step ). $( $command )"
...
$current_step = 7
Write-Host "Script: $( $script ). Step: $( $current_step ) / $( $total_step ). stop_env.ps1"
